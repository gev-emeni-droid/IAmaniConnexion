...
app.get('/evenementiel', async (c) => {
    try {
        ...
        const events = await Promise.all(rows.map(async (row) => {
            const staff = await safeQuery(c, `
                SELECT es.staff_type_id, es.count, st.name
            ...
            const assignments = await safeQuery(c, `
                SELECT a.employee_id, a.staff_type_id, 
...
app.post('/facture/:id/send-email', async (c) => {
    try {
        ...
        if (!resendKey) {
            ...
            return c.json({ error: 'Configuration envoi email manquante' }, 500);
        }
        ...
        const res = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
                from: 'IAmani <facturation@iamani.fr>',
                to: [body.to],
                subject: `Facture ${body.invoicePayload?.invoice_number || id}`,
                html: `<p>Bonjour,</p><p>Veuillez trouver ci-joint votre facture ${body.invoicePayload?.invoice_number || id}.</p><p>Cordialement,<br>L'équipe IAmani</p>`,
        ...
        if (res.ok) {
            ...
            return c.json({ error: 'Erreur envoi via Resend' }, 500);
        }
    } catch (e) { 
...
app.post('/employes', async (c) => {
    try {
        ...
        await c.env.DB.prepare('INSERT INTO employes (id, client_id, first_name, last_name, email, position, salary, hire_date, tags, phone, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
            .bind(id, ownerId, body.first_name, body.last_name, body.email, body.position, body.salary, body.hire_date, JSON.stringify(body.tags || []), body.phone, body.address).run();
        return c.json({ success: true, id });
    } catch (e) { return c.json({ error: 'Erreur' }, 500); }
});
...
app.get('/admin/support/tickets', async (c) => {
    try {
        ...
        const status = c.req.query('status'); // Si pas de status, on renvoie tout pour l'historique
        let query = `
            SELECT t.*, c.company_name as client_company 
