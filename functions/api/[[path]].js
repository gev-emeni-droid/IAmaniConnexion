...
// --- CONSTANTS & MAPPING ---
// Utiliser des alias explicites pour éviter tout décalage (column shift)
...
// Mappings manuels pour le Frontend
const mapClient = (row) => ({
    id: String(row.id || ''),
...
const mapCollab = (row) => ({
    ...row,
    created_at: toISO(row.created_at),
    modules_access: typeof row.modules_access === 'string' ? JSON.parse(row.modules_access || '[]') : (row.modules_access || [])
});
...
const adminModules = [
    ...
    { id: 'support', name: 'Support Client', enabled: true }
];
...
app.post('/auth/login', async (c) => {
    try {
        ...
        const password = body.password; // Note: In production, use bcrypt via a worker-compatible lib if needed.
        ...
        if (identifier === 'gev-emeni@outlook.fr' || identifier === 'admin' || admin) {
            ...
            const token = await sign(adminUser, getSecret(c));
            return c.json({ token, user: adminUser });
        }
        ...
        if (client) {
            ...
            const token = await sign(user, getSecret(c));
